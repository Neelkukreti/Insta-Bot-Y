...
element.click()
driver.find_element_by_css_selector("input[name='username']").send_keys("mahajan_kishan_");
#pass
element = WebDriverWait(driver, 10).until(EC.presence_of_element_located((By.CSS_SELECTOR, "input[name='password']")))
element.click()
driver.find_element_by_css_selector("input[name='password']").send_keys("mahajankishan");
#login button
...
while(linkToPostCurrent == linkToPostPrev):
    ...
    if linkToPostCurrent != linkToPostPrev:
        ...
        element.click()
        driver.find_element_by_class_name("Ypffh").send_keys("C")
